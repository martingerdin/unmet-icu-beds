...
The propensity score model is developed using the Training Data and tested using the Validation Data. The workflow contains a series of steps and the work flow should understood as linear, but iterative. The output of each step is the input for later steps. Therefore the output of the last step contain the cumulative error from unfit assumptions made the second and fourth step. The output is tested against Validation Data. By reverting back and adjusting assumptions in an iterative process, the model is continuously improved and the accuracy and validity is improved. 

<!-- ÅTERKOM inte klart bara gissat -->

The first step of PSM is to randomly split the data set into two parts, 60% and 40%. The 60% part is a Training Part used to develop and train the PSM, and the 40% part is the Validation Part. This is done because the inherent bias of the PSM method described in chapter Bias. Training Data is used in steps 2-4 and the output is then validated using Validation Data. 
