...
Fourth step is to apply this model on patients from the Validation Data set, and see whether the model correctly prediced which patients in the Validation data received ICU treatment. Using the model, the standard variable propensity score is calculated for each patient in the Validation Data set. If the patients that were admitted to the ICU in the validation set receive appropriate propensity scores by the model constructed in step 2-3, the model is valid. The propensity score determines the likelihood that a patient will receive ICU care, given their condition. 
<!-- 
...
<!-- (b) Fråga: 1. visst finns dessa svagheter i propensity score matching?
2. vad gör man åt dem? 
3. är beskrivningen av åtgärder mot detta något som hör hemma här eller i results?-->

The proportion of the subset of patients that probably should hade received ICU care based on our prediction model versus the subset that did receive ICU care is also the proportion of ICU bed shortage. 
